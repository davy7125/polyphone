...
bool SoundEngine::isEmpty()
{
    _mutexVoices.lock();
    bool bRet = _listVoices.isEmpty();
    _mutexVoices.unlock();
    return bRet;
}
...
void SoundEngine::setLoopEnabledInstance(bool isEnabled)
{
    ...
    if (voice1)
        voice1->setLoopMode(isEnabled);
    if (voice2)
        voice2->setLoopMode(isEnabled);
    _mutexVoices.unlock();
}
