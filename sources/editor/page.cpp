...
char * Page::getTextValue(char * T, quint16 champ, genAmountType genVal)
{
    ...
    case champ_keyRange:
        if (genVal.ranges.byLo == genVal.ranges.byHi)
            sprintf(T, "%s", ContextManager::keyName()->getKeyName(genVal.ranges.byLo).toStdString().c_str());
        else
            sprintf(T, "%s-%s", ContextManager::keyName()->getKeyName(genVal.ranges.byLo).toStdString().c_str(),
                    ContextManager::keyName()->getKeyName(genVal.ranges.byHi).toStdString().c_str());
        break;
    ...
    case champ_keynum: case champ_overridingRootKey:
        sprintf(T, "%s", ContextManager::keyName()->getKeyName(genVal.wAmount).toStdString().c_str());
        break;
...
genAmountType Page::getValue(QString texte, quint16 champ, bool &ok)
{
    ...
    case champ_overridingRootKey: case champ_keynum:{
        int keyNum = ContextManager::keyName()->getKeyNum(texte);
        ok = (keyNum >= 0);
