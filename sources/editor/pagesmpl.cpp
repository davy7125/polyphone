...
void PageSmpl::enleveBlanc()
{
    ...
    _sf2->endEditing("tool:smpl:removeBlank");
}

void PageSmpl::enleveBlanc(EltID id)
{
    quint32 pos = 0;
    QByteArray baData = _sf2->getData(id, champ_sampleDataFull24);
...
void PageSmpl::enleveFin()
{
}

void PageSmpl::enleveFin(EltID id)
{
...
void PageSmpl::reglerBalance()
{
    ...
    _sf2->endEditing("tool:smpl:balance");
}

void PageSmpl::transposer()
{
    if (_preparingPage)
        return;
    ...
    double rep = QInputDialog::getDouble(
                ...
                ContextManager::configuration()->getToolValue(ConfManager::TOOL_TYPE_SAMPLE, "transposition", "ton", 12.).toDouble(),
                -36, 36, 2, &ok);

    if (!ok)
        return;
    ContextManager::configuration()->setToolValue(ConfManager::TOOL_TYPE_SAMPLE, "transposition", "ton", rep);
    ...
    _sf2->endEditing("tool:smpl:transpose");
}
