...
bool SoundFont::checkInstrument(QList<int> pnums, QList<Preset*> presets, int instrIdx)
{
...
bool SoundFont::checkInstrument(QList<Preset*> presets, int instrIdx)
{
...
bool SoundFont::checkSample(QList<int> pnums, QList<Preset*> presets, QList<Instrument*> instruments, int sampleIdx)
{
    int idx = 0;
...
bool SoundFont::checkSample(QList<Preset*> presets, QList<Instrument*> instruments, int sampleIdx)
{
    int idx = 0;
    foreach(Instrument* instrument, instruments) {
...
bool SoundFont::writeCode()
{
    ...
    foreach(Instrument* instrument, instruments) {
        ...
        foreach(Zone* z, instrument->zones) {
            ...
            int n = gl.size();
            if (n) {
                fprintf(f, "Generator* geList_%d_%d[%d] = {\n      ", idx, idx2, n);
                for (int i = 0; i < n; ++i) {
                    if (i) {
                        fprintf(f, ", ");
                        if ((i % 8) == 0)
            ...
            if (sampleIdx == -1)
                fprintf(f, "IZone iz%d_%d(%d, %d, %d, %d, 0, 0, 0);\n", idx, idx2, keyLo, keyHi, veloLo, veloHi);
            else if (n)
                fprintf(f, "IZone iz%d_%d(%d, %d, %d, %d, &sample%d, %d, geList_%d_%d);  // %s\n",
                        idx, idx2, keyLo, keyHi, veloLo, veloHi, sampleIdx, n, idx, idx2,
                        samples[sampleIdx]->name);
            else
                fprintf(f, "IZone iz%d_%d(%d, %d, %d, %d, &sample%d, %d, 0);  // %s\n",
        ...
        fprintf(f, "IZone* izones%d[%d] = {\n", idx, zones);
        idx2 = 0;
        for (int i = 0; i < zones; ++i)
            fprintf(f, "      &iz%d_%d,\n", idx, idx2++);
        fprintf(f, "      };\n");
        fprintf(f, "Instrument instr%d(0, %d, izones%d);\n\n", idx, zones, idx);
        ++idx;
    }
    //
    // dump Preset[]
    ...
    foreach(Preset* p, presets) {
        ...
        foreach(Zone* z, p->zones) {
            ...
            if (keyLo == 0
                    ...
                    && veloHi == 127) {
                if (instrIdx == -1)
                    fprintf(f, "PZone pz%d_%d(0);\n", idx, idx2);
                else
                    fprintf(f, "PZone pz%d_%d(&instr%d);\n", idx, idx2, instrIdx);
            }
            else {
                //abort();

                if (instrIdx == -1)
                    fprintf(f, "PZone pz%d_%d(%d, %d, %d, %d, 0);\n", idx, idx2, keyLo, keyHi, veloLo, veloHi);
                else
                    fprintf(f, "PZone pz%d_%d(%d, %d, %d, %d, &instr%d);\n", idx, idx2, keyLo, keyHi, veloLo, veloHi, instrIdx);
            }
            if (_smallSf)
                break;
            ++idx2;
        }
        fprintf(f, "PZone* pzones%d[%d] = {\n", idx, zones);
        idx2 = 0;
        for (int i = 0; i < zones; ++i) {
            fprintf(f, "      &pz%d_%d,\n", idx, idx2);
            ++idx2;
        ...
        fprintf(f, "Preset preset%d(%d, %d, 0, %d, pzones%d);  // %s\n\n",
                idx, p->preset, p->bank, zones, idx, p->name);
        ++idx;
    }


    fprintf(f, "Preset* sfPresets[%d] = {\n", presets.size());
    for(int idx = 0; idx < presets.size(); ++idx)
        fprintf(f, "      &preset%d,   // %s\n", idx, presets[idx]->name);
    fprintf(f, "      };\n");

    fprintf(f, "SFont _buildinSf(%d, sfPresets);\n", presets.size());
    ...
    foreach(Instrument* instrument, instruments) {
        ...
        foreach(Zone* z, instrument->zones) {
            ...
            if (sampleIdx == -1)
                ...
                fprintf(f, "IZone iz%d_%d(%d, %d, %d, %d, &sample%d, %d, geList_%d_%d);\n",
                        idx, idx2, keyLo, keyHi, veloLo, veloHi, sampleIdx, n, idx, idx2);
            ++idx2;
        }
