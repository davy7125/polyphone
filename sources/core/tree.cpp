...
Tree::menuClicDroit::menuClicDroit(MainWindowOld *mainWindow)
{
...
void Tree::init(MainWindowOld *mainWindow, SoundfontManager * sf2, QComboBox * comboSf2)
{
...
void Tree::displaySample(int idSf2, int index, bool repercute)
{
    ...
    if (repercute)
    {
        ...
        QList<int> nbInst = _sf2->getSiblings(idInst);
        foreach (int i, nbInst)
        {
            idInst.indexElt = i;
            idInstSmpl.indexElt = i;

            QList<int> nbInstSmpl = _sf2->getSiblings(idInstSmpl);
            foreach (int j, nbInstSmpl)
            {
...
void Tree::displayInstrument(int idSf2, int index, bool repercuteSmpl, bool repercutePrst)
{
    ...
    if (repercuteSmpl)
    {
        ...
        idInstSmpl.typeElement = elementInstSmpl;
        QList<int> nbElement = _sf2->getSiblings(idInstSmpl);
        foreach (int i, nbElement)
        {
    ...
    if (repercutePrst)
    {
        ...
        QList<int> nbPrst = _sf2->getSiblings(idPrst);
        foreach (int i, nbPrst)
        {
            idPrst.indexElt = i;
            idPrstInst.indexElt = i;
            QList<int> nbPrstInst = _sf2->getSiblings(idPrstInst);
            foreach (int j, nbPrstInst)
            {
...
void Tree::displayPreset(int idSf2, int index, bool repercute)
{
    ...
    if (repercute)
    {
        ...
        idPrstInst.typeElement = elementPrstInst;
        QList<int> nbElement = _sf2->getSiblings(idPrstInst);
        foreach (int i, nbElement)
        {
...
void Tree::select(QList<EltID> ids)
{
    bool previousBlockState = this->blockSignals(true);
    this->selectNone();
    foreach (EltID id, ids)
        this->select(id);
    this->blockSignals(previousBlockState);
}
