...
ImportSfz::ImportSfz(SoundfontManager * sf2) :
    _sf2(sf2),
...
void ImportSfz::import(QString fileName, int * numSf2, bool &success, QString &error)
{
    ...
    if (inputFile.open(QIODevice::ReadOnly))
    {
        ...
        _sf2->set(idSf2, champ_ICMT, QObject::trUtf8("Sf2 importé depuis un fichier sfz par Polyphone"));
        ...
        _sf2->set(idPrst, champ_wPreset, val);
        ...
        for (int i = 0; i < _listeEnsembles.size(); i++)
        {
            ...
            if (velMin <= velMax)
            {
                ...
                _sf2->set(idPrstInst, champ_velRange, val);
            }
            ...
            if (!_sf2->isSet(idInst, champ_initialAttenuation))
            {
                foreach (int i, _sf2->getSiblings(idInstSmpl))
                {
                    ...
                    if (_sf2->get(idInstSmpl, champ_initialAttenuation).wValue == 0)
                        _sf2->reset(idInstSmpl, champ_initialAttenuation);
                }
            }
            if (!_sf2->isSet(idInst, champ_fineTune))
            {
                foreach (int i, _sf2->getSiblings(idInstSmpl))
                {
                    ...
                    if (_sf2->get(idInstSmpl, champ_fineTune).wValue == 0)
                        _sf2->reset(idInstSmpl, champ_fineTune);
                }
            }
            ...
            foreach (int i, _sf2->getSiblings(idInstSmpl))
            {
                ...
                if (_sf2->get(idInstSmpl, champ_sampleModes).wValue == typeLoop)
                    _sf2->reset(idInstSmpl, champ_sampleModes);
            }
            if (!_sf2->isSet(idInst, champ_exclusiveClass))
            {
                foreach (int i, _sf2->getSiblings(idInstSmpl))
                {
                    ...
                    if (_sf2->get(idInstSmpl, champ_exclusiveClass).wValue == 0)
                        _sf2->reset(idInstSmpl, champ_exclusiveClass);
                }
            }

            // Suppression keyrange et velocity range de la division globale de l'instrument
            _sf2->reset(idInst, champ_velRange);
            _sf2->reset(idInst, champ_keyRange);
        }
    }
...
void EnsembleGroupes::decode(SoundfontManager * sf2, EltID idInst, QString pathSfz)
{
...
QList<int> GroupeParametres::getSampleIndex(SoundfontManager *sf2, EltID idElt, QString pathSfz) const
{
