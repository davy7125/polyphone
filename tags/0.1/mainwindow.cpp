...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent, Qt::Window | Qt::WindowCloseButtonHint |
                                                      ...
                                                      Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::CustomizeWindowHint),
    ...
    configuration(this), help(this)
{
    ...
    this->audio->connect(this, SIGNAL(initAudio(int)), SLOT(initAudio(int)));
    this->audio->moveToThread(&this->audioThread);
    this->audioThread.start();
    this->audioThread.setPriority(QThread::LowPriority);
    emit(initAudio(this->configuration.getAudioIndex()));
    ...
    page_sf2 = new Page_Sf2(this, ui->arborescence, ui->stackedWidget, this->sf2, this->audio);
    ...
    this->audio->connect(this->ui->actionAnnuler, SIGNAL(triggered()), SLOT(finished2()), Qt::BlockingQueuedConnection);
    this->audio->connect(this->ui->actionR_tablir, SIGNAL(triggered()), SLOT(finished2()), Qt::BlockingQueuedConnection);
}
MainWindow::~MainWindow()
{
    // Fin du thread (200 ms pour quitter)
...
void MainWindow::ouvrir()
{
    ...
    fileNameTmp = QFileDialog::getOpenFileName(this, tr("Ouvrir une soundfont"), fileNameTmp, tr("Fichier .sf2 (*.sf2)"));
        ...
        if (ok && !text.isEmpty())
        {
            ...
            else
            {
                ...
                for (int i = 0; i < nb; i++)
                {
                    ...
                    note = sf2->get(ID, champ_byOriginalPitch).wValue;
                    char str2[20];
        ...
        for (int j = 0; j < nChannels; j++)
        {
                    ...
                    (nChannels == 1 && indexL == -1)) // Si pas ignorer
            {
                     ...
                     (nChannels == 1 && indexL != -1)) && (replace == 2 || replace == 1))
                {
                    ...
                    QByteArray data = son->getData(16);
                    this->sf2->set(id, champ_sampleData16, data);
                    data = son->getData(8);
                    this->sf2->set(id, champ_sampleData24, data);
                }
                else
                {
                    // Ajout d'un sample
