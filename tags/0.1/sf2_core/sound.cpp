...
QByteArray Sound::getData(WORD wBps)
{
    ...
    case 24:
        ...
        if (!this->smpl.isEmpty() && !this->sm24.isEmpty() && this->info.wBpsFile >= 24)
        {
            ...
            for (int i = 0; i < (int)this->info.dwLength; i++)
            {
                baRet[3*i+1] = this->sm24.at(i);
                baRet[3*i+1] = this->smpl.at(2*i);
                baRet[3*i+2] = this->smpl.at(2*i+1);
            }
        }
...
QByteArray Sound::bpsConversion(QByteArray baData, WORD wBpsInit, WORD wBpsFinal)
{
    ...
    case 16:
        ...
        case 8:
            ...
            for (int i = 0; i < size/2; i++)
                baRet[i] = baData.at(2*i+1);
            break;
        case 16:
            baRet = baData;
            break;
        case 24:
            baRet.resize(3*size/2);
            for (int i = 0; i < size/2; i++)
            {
                baRet[3*i] = 0;
                baRet[3*i+1] = baData.at(2*i);
                baRet[3*i+2] = baData.at(2*i+1);
            }
            break;
        case 32:
            baRet.resize(4*size/2);
            for (int i = 0; i < size/2; i++)
            {
                baRet[4*i] = 0;
                baRet[4*i+1] = 0;
                baRet[4*i+2] = baData.at(2*i);
                baRet[4*i+3] = baData.at(2*i+1);
            }
            break;
        }
        break;
    case 24:
        ...
        case 824:
            ...
            for (int i = 0; i < size/3; i++)
                baRet[i] = baData.at(3*i);
            break;
        case 8:
            baRet.resize(size/3);
            for (int i = 0; i < size/3; i++)
                baRet[i] = baData.at(3*i+2);
            break;
        case 16:
            baRet.resize(2*size/3);
            for (int i = 0; i < size/3; i++)
            {
                baRet[2*i] = baData.at(3*i+1);
                baRet[2*i+1] = baData.at(3*i+2);
            }
            break;
        case 24:
            baRet = baData;
            break;
        case 32:
            baRet.resize(4*size/3);
            for (int i = 0; i < size/3; i++)
            {
                baRet[4*i] = 0;
                baRet[4*i+1] = baData.at(3*i);
                baRet[4*i+2] = baData.at(3*i+1);
                baRet[4*i+3] = baData.at(3*i+2);
            }
            break;
        }
        break;
    case 32:
        ...
        case 824:
            ...
            for (int i = 0; i < size/4; i++)
                baRet[i] = baData.at(4*i+1);
            break;
        case 8:
            baRet.resize(size/4);
            for (int i = 0; i < size/4; i++)
                baRet[i] = baData.at(4*i+3);
            break;
        case 16:
            baRet.resize(2*size/4);
            for (int i = 0; i < size/4; i++)
            {
                baRet[2*i] = baData.at(4*i+2);
                baRet[2*i+1] = baData.at(4*i+3);
            }
            break;
        case 24:
            baRet.resize(3*size/4);
            for (int i = 0; i < size/4; i++)
            {
                baRet[3*i] = baData.at(4*i+1);
                baRet[3*i+1] = baData.at(4*i+2);
                baRet[3*i+2] = baData.at(4*i+3);
            }
            break;
        case 32:
            baRet = baData;
...
QByteArray Sound::from2MonoTo1Stereo(QByteArray baData1, QByteArray baData2, WORD wBps, bool bigEndian)
{
    ...
    if (wBps == 32)
    {
        if (bigEndian)
        {
            for (int i = 0; i < size/4; i++)
            {
                baRet[8*i] = baData1.at(4*i+3);
                baRet[8*i+1] = baData1.at(4*i+2);
                baRet[8*i+2] = baData1.at(4*i+1);
                baRet[8*i+3] = baData1.at(4*i);
                baRet[8*i+4] = baData2.at(4*i+3);
                baRet[8*i+5] = baData2.at(4*i+2);
                baRet[8*i+6] = baData2.at(4*i+1);
                baRet[8*i+7] = baData2.at(4*i);
            }
        }
        else
        {
            for (int i = 0; i < size/4; i++)
            {
                baRet[8*i] = baData1.at(4*i);
                baRet[8*i+1] = baData1.at(4*i+1);
                baRet[8*i+2] = baData1.at(4*i+2);
                baRet[8*i+3] = baData1.at(4*i+3);
                baRet[8*i+4] = baData2.at(4*i);
                baRet[8*i+5] = baData2.at(4*i+1);
                baRet[8*i+6] = baData2.at(4*i+2);
                baRet[8*i+7] = baData2.at(4*i+3);
            }
        }
    }
    else if (wBps == 24)
    {
        if (bigEndian)
        {
            for (int i = 0; i < size/3; i++)
            {
                baRet[6*i] = baData1.at(3*i+2);
                baRet[6*i+1] = baData1.at(3*i+1);
                baRet[6*i+2] = baData1.at(3*i);
                baRet[6*i+3] = baData2.at(3*i+2);
                baRet[6*i+4] = baData2.at(3*i+1);
                baRet[6*i+5] = baData2.at(3*i);
            }
        }
        else
        {
            for (int i = 0; i < size/3; i++)
            {
                baRet[6*i] = baData1.at(3*i);
                baRet[6*i+1] = baData1.at(3*i+1);
                baRet[6*i+2] = baData1.at(3*i+2);
                baRet[6*i+3] = baData2.at(3*i);
                baRet[6*i+4] = baData2.at(3*i+1);
                baRet[6*i+5] = baData2.at(3*i+2);
            }
        }
    }
    else
    {
        if (bigEndian)
        {
            for (int i = 0; i < size/2; i++)
            {
                baRet[4*i] = baData1.at(2*i+1);
                baRet[4*i+1] = baData1.at(2*i);
                baRet[4*i+2] = baData1.at(2*i+1);
                baRet[4*i+3] = baData2.at(2*i);
            }
        }
        else
        {
            for (int i = 0; i < size/2; i++)
            {
                baRet[4*i] = baData1.at(2*i);
                baRet[4*i+1] = baData1.at(2*i+1);
                baRet[4*i+2] = baData1.at(2*i);
                baRet[4*i+3] = baData2.at(2*i+1);
            }
        }
    }
    return baRet;
...
QByteArray Sound::enleveBlanc(QByteArray baData, double seuil, WORD wBps, qint32 &pos)
{
    // Conversion 32 bits si nécessaire
    if (wBps != 32)
        baData = bpsConversion(baData, wBps, 32);
    ...
    while ((pos < baData.size()/4 - 1) && (data2[pos] < seuil * median)) pos++;
    // Saut
    if (pos < baData.size()/4 - 1)
