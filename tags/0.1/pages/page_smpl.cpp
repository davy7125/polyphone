    ...
    Page(parent), ui(new Ui::Page_Smpl)
{
    ...
    // Connexion d'audio vers le graphe
    this->ui->graphe->connect(this->audio, SIGNAL(setCurrentPos(int)), SLOT(setCurrentSample(int)));
    // Connexion d'audio vers finished
    this->connect(this->audio, SIGNAL(sigFinished()), SLOT(lecteurFinished()));
    this->connect(this->audio, SIGNAL(sigFinished2()), SLOT(lecteurFinished2()));
    // Connexion à audio des méthodes d'initialisation, start et stop
    this->audio->connect(this, SIGNAL(setData(QByteArray*,QByteArray*,quint32)),
                         SLOT(setData(QByteArray*,QByteArray*,quint32)),
                         Qt::BlockingQueuedConnection);
    this->audio->connect(this, SIGNAL(setPlayParameter(int,int,int,int,int,bool,bool,int,int,int,int)),
                         SLOT(setPlayParameter(int,int,int,int,int,bool,bool,int,int,int,int)),
                         Qt::BlockingQueuedConnection);
    this->audio->connect(this, SIGNAL(start()), SLOT(start()));
    this->audio->connect(this->tree, SIGNAL(itemSelectionChanged()), SLOT(finished3())); // arrêt immédiat avec possibilité de repartir
    this->audio->connect(this, SIGNAL(stop()), SLOT(stop()), Qt::BlockingQueuedConnection);
    // Connexion à audio des contrôles de la page sample
    this->audio->connect(this->ui->spinStartLoop, SIGNAL(valueChanged(int)), SLOT(setStartLoop(int)));
    this->audio->connect(this->ui->spinEndLoop, SIGNAL(valueChanged(int)), SLOT(setEndLoop(int)));
    this->audio->connect(this->ui->checkLectureBoucle, SIGNAL(stateChanged(int)), SLOT(setBoucle(int)));
    this->audio->connect(this->ui->checkLectureSinus, SIGNAL(stateChanged(int)), SLOT(setSinus(int)));
    this->audio->connect(this->ui->spinTune, SIGNAL(valueChanged(int)), SLOT(setTune(int)));
    this->audio->connect(this->ui->spinRootKey, SIGNAL(valueChanged(int)), SLOT(setRootKey(int)));
    this->audio->connect(this->ui->sliderLectureVolume, SIGNAL(valueChanged(int)), SLOT(setVolume(int)));
    this->audio->connect(this->ui->checkLectureLien, SIGNAL(stateChanged(int)), SLOT(setStereo(int)));
...
void Page_Smpl::afficher()
{
    ...
    this->ui->graphe->setData(baData, dwSmplRate);
...
void Page_Smpl::lecture()
{
    if (this->ui->pushLecture->isChecked())
    {
        // Lecture du sample
        EltID id = this->tree->getID(0);
        QByteArray baData = Sound::bpsConversion(this->sf2->getData(id, champ_sampleDataFull24), 24, 32);
        // Lien ?
        SFSampleLink typeLien = this->sf2->get(id, champ_sfSampleType).sfLinkValue;
        QByteArray baData2;
        EltID id2 = id;
        ...
        case monoSample: case RomMonoSample:
            baData2 = baData;
            startLoop1 = this->ui->spinStartLoop->value();
            endLoop1 = this->ui->spinEndLoop->value();
            startLoop2 = startLoop1;
            endLoop2 = endLoop1;
            break;
        case rightSample: case RomRightSample: case linkedSample: case RomLinkedSample:
            id2.indexElt = this->sf2->get(id, champ_wSampleLink).wValue;
            baData2 = Sound::bpsConversion(this->sf2->getData(id2, champ_sampleDataFull24), 24, 32);
            if (this->sf2->get(id, champ_dwSampleRate).dwValue < this->sf2->get(id2, champ_dwSampleRate).dwValue)
            {
                dwSmplRate = this->sf2->get(id2, champ_dwSampleRate).dwValue;
                Sound::resampleMono(baData, this->sf2->get(id, champ_dwSampleRate).dwValue, dwSmplRate, 32, 0, d);
            }
            else if (this->sf2->get(id2, champ_dwSampleRate).dwValue < this->sf2->get(id, champ_dwSampleRate).dwValue)
            {
                Sound::resampleMono(baData2, this->sf2->get(id2, champ_dwSampleRate).dwValue, dwSmplRate, 32, 0, d);
            }
            startLoop1 = this->ui->spinStartLoop->value();
            endLoop1 = this->ui->spinEndLoop->value();
            startLoop2 = this->sf2->get(id2, champ_dwStartLoop).dwValue;
            endLoop2 = this->sf2->get(id2, champ_dwEndLoop).dwValue;
            typeSmpl = 1;
            break;
        case leftSample: case RomLeftSample:
            baData2 = baData;
            id2.indexElt = this->sf2->get(id, champ_wSampleLink).wValue;
            baData = Sound::bpsConversion(this->sf2->getData(id2, champ_sampleDataFull24), 24, 32);
            if (this->sf2->get(id, champ_dwSampleRate).dwValue < this->sf2->get(id2, champ_dwSampleRate).dwValue)
            {
                dwSmplRate = this->sf2->get(id2, champ_dwSampleRate).dwValue;
                Sound::resampleMono(baData, this->sf2->get(id, champ_dwSampleRate).dwValue, dwSmplRate, 32, 0, d);
            ...
            {
                dwSmplRate = this->sf2->get(id, champ_dwSampleRate).dwValue;
                Sound::resampleMono(baData2, this->sf2->get(id2, champ_dwSampleRate).dwValue, dwSmplRate, 32, 0, d);
            }
            startLoop1 = this->sf2->get(id2, champ_dwStartLoop).dwValue;
            endLoop1 = this->sf2->get(id2, champ_dwEndLoop).dwValue;
            startLoop2 = this->ui->spinStartLoop->value();
            endLoop2 = this->ui->spinEndLoop->value();
            typeSmpl = 2;
            break;
        }
        // Initialisation
        emit(setData(&baData, &baData2, dwSmplRate));
        emit(setPlayParameter(startLoop1, endLoop1, typeSmpl, startLoop2, endLoop2,
                              this->ui->checkLectureBoucle->isChecked(), this->ui->checkLectureSinus->isChecked(),
                              this->ui->spinTune->value(), this->ui->spinRootKey->value(),
                              this->ui->sliderLectureVolume->value(),
                              this->ui->checkLectureLien->isChecked()));
        // Désactivations
...
void Page_Smpl::lecteurFinished2()
{
    if (this->lectureEnCours && this->tree->getSelectedItemsNumber())
    {
        if (this->tree->getID(0).typeElement == elementSmpl)
        {
            // La lecture était en cours lors du clic sur l'arborescence, on relance
            this->lecture();
        }
        else
            this->lecteurFinished();
    }
    else
        this->lecteurFinished();
}
